...
void main() {
  ...
  float flicker = 0.84 + aSeed * 0.34 + uHigh * 0.2 * step(1.5, population);
  ...
  vEnergy = uOverall * 0.68 + uHigh * 0.25 * step(1.5, population) + density * 0.16;
...
void main() {
  ...
  vec3 ice = mix(vec3(0.69, 0.78, 0.86), vec3(0.965, 0.99, 1.0), clamp(core + vEnergy * 0.38, 0.0, 1.0));
  gl_FragColor = vec4(ice * (0.86 + core * 0.48 + vEnergy * 0.32), alpha);
}
`;
...
void main() {
  ...
  float breath = sin(uTime * 0.31 + phase) * 0.018 + uBass * 0.025;
  vec3 displaced = position * (1.0 + breath * motion);
  displaced += tangent * flow * (0.035 + aPopulation * 0.018) * motion * (1.0 + uMid * 0.4);
  displaced += normal * sin(uTime * 1.7 + phase) * uHigh * 0.008 * motion;
